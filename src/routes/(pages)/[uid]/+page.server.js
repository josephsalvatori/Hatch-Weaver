...
export async function load({ cookies, fetch, parent, params, request, url }) {
	...
	if(parentData.settings?.settings?.data?.alternative_landing_page?.uid === params.uid) redirect(302, `/`);
	...
	/** If parent page */
	if(response?.data?.parent_page?.uid) redirect(301, `/${response.data.parent_page.uid}/${params.uid}/`);
