...
export function accepts(file, acceptedFiles) {
	...
	const acceptedFileArray = Array.isArray(acceptedFiles) ? acceptedFiles : acceptedFiles.split(",");
	const fileName = file.name || "";
	const mimeType = (file.type || "").toLowerCase();
	const baseMimeType = mimeType.replace(/\/.*$/, "");

	return acceptedFileArray.some(type => {
...
// File Errors
export const getInvalidTypeRejectionError = (accept) => {
...
export const getTooLargeRejectionError = (maxSize) => {
	return {
		code: FILE_TOO_LARGE,
...
export const getTooSmallRejectionError = (minSize) => {
	return {
		code: FILE_TOO_SMALL,
...
export function fileMatchSize(file, minSize, maxSize) {
	...
	console.log("match size", minSize, maxSize, isDefined(file.size), [true, null]);

	if(!isDefined(file.size)) return [true, null];
